...
def main(_argv):
    ...
    # img = cv2.imread('./data/video/cafetefria_mask.png')
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            thebox_area = int(bbox[2]-bbox[0]) * int(bbox[3]-bbox[1])
            thebox= cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, 2)
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name)+len(str(track.track_id)))*17, int(bbox[1])), color, -1)
            cv2.putText(frame, class_name + "-" + str(track.track_id),(int(bbox[0]), int(bbox[1]-10)),0, 0.75, (255,255,255),2)
            cv2.putText(frame,'size ' + str(thebox_area) + "-" + str(track.track_id),(int(bbox[0]), int(bbox[1]-20)),0, 0.75, (255,255,255),2)
            cv2.putText(frame,'low y ' + str(int(bbox[3])) + "-" + str(track.track_id),(int(bbox[0]), int(bbox[1]+40)),0, 0.75, (255,255,255),2)
            # cv2.putText(frame,'chance'+ str(scores) + "-" + str(track.track_id),(int(bbox[0]), int(bbox[1]-30)),0, 0.75, (255,255,255),2)


            ##tinn size boxcheck
            if (int(bbox[3]<= 1050 and thebox_area >15000)) and  (( thebox_area > 22500 or thebox_area < 115000)  and  ( thebox_area > 185000 or thebox_area < 240000)) : #upper part far from cam
                #nui estimate foot area
                # cv2.rectangle(try_frame, (int(bbox[0] ), int(int(bbox[3]) - int(int(bbox[3] - bbox[1])/7))) , (int(bbox[2]*3/4), int(bbox[3])), (255, 255, 255), -1)
                cv2.rectangle(try_frame, (int(bbox[0]+((bbox[2]-bbox[0])/3)) , int(int(bbox[3]) - int(int(bbox[3] - bbox[1])/7))) , (int(bbox[2]-int((bbox[2]-bbox[0])/4)), int(bbox[3])), (255, 255, 255), -1)
                #int((bbox[2]-bbox[0])/4)
        ...
        if first_iteration_indicator == 1:
            ...
            # th1 = cv2.bitwise_and(img, th1)
            th1 = cv2.bitwise_and(img_erode, th1)
